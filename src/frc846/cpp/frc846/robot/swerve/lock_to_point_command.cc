...
void LockToPointCommand::Execute() {
  ...
  if (r_vec.magnitude() <
    ...
    frc846::math::VectorND<units::feet_per_second, 2> speed_vector = {
        1_fps * lock_gains.calculate(r_vec[0].to<double>(), 0.0,
                    drivetrain_->GetReadings()
                        .estimated_pose.velocity[0]
                        .to<double>(),
                    0.0),
        1_fps * lock_gains.calculate(r_vec[1].to<double>(), 0.0,
                    drivetrain_->GetReadings()
                        .estimated_pose.velocity[1]
                        .to<double>(),
                    0.0)};
