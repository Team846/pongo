...
class GenericVerticalArmSubsystem
 ...
 public:
  GenericVerticalArmSubsystem(
      ...
      : configs_(configs), mmtype_(mmtype), constr_params_(constr_params) {}
  ...
  double CalculateDutyCycle(units::degree_t arm_position,
    ...
    units::newton_meter_t gravity_torque =
        configs_.arm_mass_ * configs_.center_of_mass_ *
        frc846::math::physics.g *
    ...
    double duty_cycle = frc846::control::base::MotorGains::calculate(
        position_error, 0, current_velocity.to < double(),
        total_torque.to<double>());
    return std::max(std::min(duty_cycle, configs.peak_output_forward),
                    configs.peak_output_reverse);
  }
